...
namespace MoneyTransfer
{
    internal class Person
    {
        ...
        public bool TransferMoney(Person receiver, int amountToTransfer)
        {
            ...
            if (wallet.money >= amountToTransfer)
            {
                wallet.money -= amountToTransfer;
                receiver.AcceptMoney(amountToTransfer);
                Console.WriteLine($"${amountToTransfer} was subtracted from Pablo's wallet and deposited into Maria's wallet.");
                Console.WriteLine();
                return true;
            }
            else
            {
                Console.WriteLine("Transaction failed due to lack of funds.");
                Console.WriteLine();
                return false;
            }
        }
